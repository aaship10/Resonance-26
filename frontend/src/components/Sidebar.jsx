...
import { AuthContext } from '../context/AuthContext'; // 1. Import Global State

// UI REMAINS 100% UNTOUCHED
const SidebarItem = ({ icon, label, path, aliases = [] }) => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Expanded logic slightly to match dynamic editor routes (e.g., /editor/1)
  const active = location.pathname.startsWith(path) || aliases.some(alias => location.pathname.startsWith(alias));
...
const Sidebar = () => {
  const navigate = useNavigate();
  const { user, logout } = useContext(AuthContext); // 2. Consume Global State
  ...
  return (
    <aside className="h-screen w-[220px] fixed left-0 top-0 z-40 bg-[#f3eff6] border-r border-[#cbbec9]/10 shadow-[10px_0_30px_rgba(53,41,59,0.03)] hidden lg:flex flex-col py-20 shrink-0">
      ...
      <nav className="space-y-1 flex-1 pr-6">
        ...
        {/* 3. Conditional Navigation based on the User's Role */}
        
        {/* Analyst & Approver Shared Views */}
        {user?.role !== 'Admin' && (
        ...
        {user?.role === 'Analyst' && (
          <>
            ...
            <SidebarItem icon="manage_search" label="Investigations" path="/investigation" aliases={['/generating', '/editor']} />
          </>
        )}
        ...
        {user?.role === 'Admin' && (
          <>
            <SidebarItem icon="admin_panel_settings" label="System Config" path="/admin/settings" />
      ...
      <div className="mt-auto space-y-1 pb-4 px-6">
        ...
        {/* 4. Actually execute the secure logout function to clear the JWT token */}
